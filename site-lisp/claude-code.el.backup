...
(defvar claude-code--in-window-config-change nil
  "Guard variable to prevent recursive window configuration changes.")
...
configuration changes (e.g., when minibuffer opens/closes)."
  (unless claude-code--in-window-config-change
    (let ((claude-code--in-window-config-change t))
    ...
    (with-current-buffer buffer
      ...
      (let ((process-adaptive-read-buffering nil))
        (condition-case err
          ...
          (error
           (message "Failed to start Claude: %s" (error-message-string err))
           (when (buffer-live-p buffer)
             (kill-buffer buffer))
           (error "Failed to start Claude Code - check if 'claude' command is available"))))
      ...
      ;; Use async timer instead of blocking sleep-for to avoid hanging Emacs
